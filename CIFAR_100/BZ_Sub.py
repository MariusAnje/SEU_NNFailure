...
if __name__=='__main__':
    ...
    parser.add_argument('--data', action='store', default='./data/',
            help='dataset path')
    parser.add_argument('--pretrained', action='store', default='models/56.T4.pth.tar',
    ...
    for SubNum in range(0,1):
        indices = np.load("SubClass/subHalf100C%d.npy"%(SubNum))
        testloader = torch.utils.data.DataLoader(torch.utils.data.Subset(test_dataset, indices),
        ...
        if args.evaluate:
            ...
            for find_key in keyList:
                ...
                np.save(find_key+'.'+args.arch+'.zeroSub%d'%(SubNum), save)
