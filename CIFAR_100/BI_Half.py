...
from models import resnetHalf as resnet
import torchvision
...
def test(i=None, key=None, shape=[], rand = False, bypass = False, randFactor = None, memoryData = None, same = False):
    global best_acc
    test_loss = 0
    correct = 0
...
if __name__=='__main__':
    ...
    parser.add_argument('--data', action='store', default='./data/',
            help='dataset path')
    parser.add_argument('--pretrained', action='store', default='res56.best.pth.tar',
    ...
    if args.testonly:
        print(test(same = same,memoryData = memoryData))
        exit()
    ...
    if args.evaluate:
        ...
        for find_key in keyList:
            ...
            np.save(find_key+'.'+args.arch+'.'+args.filename+'.half', save)
