        ...
        if rand:
            if ((int(i/(size2*size3))%int(size1)) == torch.randint(0,size1-1,[1]) or bypass):
                ...
                if (flag):
                    ...
                    bin_op.binarization()
                    state_dict = model.state_dict()
                    state_dict[key][int(i/size1/size2/size3)][int(i/size2/size3%size1)][int(i/size3%size2)][int(i%size3)] = 0
                else:
                    return 100
            else:
                return 100
    ...
    if len(shape) == 2:
        ...
        if rand:
            if (int(int(i)%randFactor) == torch.randint(0,randFactor-1,[1])):
                ...
                model.eval()
                bin_op.binarization()
                state_dict[key][int(i/size)][i%size] = 0
            else:
                return 100
        else:
            state_dict[key][int(i/size)][i%size] = 0
...
if __name__=='__main__':
    ...
    for SubNum in range(0,10):
        indices = np.load("subHalf%d.npy"%(SubNum))
        ...
        if args.evaluate:
            ...
            np.save(find_key+'.BZsub%d'%(SubNum), save)
